    ...
    return (
      <div className="mx-auto w-full max-w-[1280px] px-6 py-28 lg:px-8 text-center">
        ...
        <p className="text-gray-500 mb-6">
          {error || "Không tìm thấy dữ liệu đơn đặt hàng."}
        </p>
        <Link
          to="/orders"
          className="rounded-full px-6 py-2.5 text-xs font-bold text-white shadow"
  ...
  const steps = [
    {
      ...
      icon: "shopping_bag",
      desc: "Đơn hàng mới đã được khởi tạo",
    },
    {
      label: "Đã xác nhận",
      icon: "verified",
      desc: "Shop đã phê duyệt đơn hàng",
    },
    {
      label: "Chuẩn bị hàng",
      icon: "inventory_2",
      desc: "Shop đang chuẩn bị đóng gói hàng hóa",
    },
    {
      label: "Đang giao hàng",
      icon: "local_shipping",
      desc: "Đơn hàng đang trên đường tới bạn",
    },
    {
      label: "Đã giao thành công",
      icon: "task_alt",
      desc: "Giao hàng thành công hoàn tất",
    },
  ];
    ...
    <div className="mx-auto w-full max-w-[1280px] px-6 py-28 lg:px-8">
      ...
      {order.status === "cancelled" ? (
        <div className="mb-8 rounded-2xl bg-red-50 p-6 border border-red-100 flex items-start gap-4">
          <span className="material-symbols-outlined text-3xl text-red-500 shrink-0">
            cancel
          </span>
          <div>
            ...
            <p className="text-sm text-red-700">
              Hệ thống đã tự động hoàn trả số lượng hàng vào kho. Tiền thanh
              toán của bạn (nếu có) sẽ được xử lý theo quy trình hoàn tiền.
            </p>
            {order.adminNote && (
        ...
        <div className="mb-8 rounded-2xl bg-orange-50 p-6 border border-orange-100 flex items-start gap-4">
          <span className="material-symbols-outlined text-3xl text-orange-500 shrink-0 animate-pulse">
            hourglass_top
          </span>
          <div>
            ...
            <p className="text-sm text-orange-700">
              Yêu cầu hủy đơn hàng của bạn đang được Shop xem xét. Shop đang
              đóng gói hàng nên việc tự hủy trực tiếp đã bị khóa. Vui lòng chờ
              phản hồi hoặc liên hệ hỗ trợ.
            </p>
          </div>
        </div>
      ) : (
        timeLeftStr && (
          <div className="mb-8 rounded-2xl bg-blue-50 p-6 border border-blue-100 flex items-start gap-4 shadow-sm">
            <span
              className="material-symbols-outlined text-3xl text-blue-600 shrink-0 animate-spin"
            ...
            <div>
              <h3 className="text-base font-bold text-blue-900 mb-1">
                Đơn hàng trong giai đoạn chờ 30 phút
              </h3>
              <p className="text-sm text-blue-700">
                Đơn hàng sẽ tự động chuyển sang trạng thái **Đã xác nhận** sau{" "}
                <span className="font-extrabold text-blue-900 bg-blue-100 px-2 py-0.5 rounded-md font-mono">
                  {timeLeftStr}
                </span>{" "}
                phút nữa. Trong thời gian này, bạn hoàn toàn có thể **hủy trực
                tiếp** mà không cần chờ duyệt!
              </p>
            </div>
          </div>
        )
          ...
          <div className="hidden md:flex items-start justify-between relative">
            ...
            {steps.map((step, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex flex-col items-center text-center w-1/5 z-10 relative"
                >
                  <div
                  ...
                  <p
                    className={`mt-3 text-xs font-bold ${isCompleted ? "text-gray-900" : "text-gray-400"}`}
                  >
                    {step.label}
                  </p>
              ...
              {order.shippingSnapshot ? (
                <div className="text-sm text-gray-600 space-y-1.5">
                  ...
                  <p className="text-xs leading-relaxed mt-1 text-gray-500">
                    {order.shippingSnapshot.streetAddress}, Phường{" "}
                    {order.shippingSnapshot.ward}, Quận{" "}
